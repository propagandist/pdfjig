...
★★ **置き換える前に、書けた中身をディスクへ届けさせる**（#219）。改名はメタデータであり、中身の書き込みより先に確定しうる——届く前に置き換えて控えを消すと、直後の電源断で**新旧どちらも失いうる**（NTFS の順序保証からの読み。再現はしていない）。**`pdf-core` の書き出しはすべて、閉じる前に書いたハンドルで届けさせる**（`DurableSave`。上の「既存の出力を拒む」と並ぶ `pdf-core` 全体の契約）。名前で開き直す形は、出力先の権限や他のプロセスの共有モードで**保存を新たに失敗させる**ので採らない。★ **届けさせられなくても、書き出しは失敗させない**——吐き出しを受け付けない場所（転送ドライブ・仮想マシンの共有フォルダなど。どこかは確かめていない）で失敗させると、以前は通った保存も分割も必ず失敗する。**その場所では v0.1.x と同じ守りに戻るだけである**（判断は利用者に確かめた）。★ **分割も届けさせる**——置き換えないので得るものは無いが、契約を呼ぶ側で分けない。1 ファイルあたり SSD で数 ms 延びる（#219 の門で実測）。
