...
★ **分割も届けさせる**——置き換えないので得るものは無いが、`pdf-core` の契約を呼ぶ側で分けない。**遅くなる**：SSD で 1 ファイルあたり数 ms、7200rpm の HDD で 1 ページずつ 200 ファイルに分けると 1.0 秒が 7.6 秒になった（2026-09-25 実測。#219 の門）。
...
**★ ただし、数え上げた 6 つのうち書き換えてよいのは 3 つだけである**——しおり・文書情報・ページラベル。**それ以外は書き換えない。** 理由つきの表（正と負の両方、および出力に含めないページを指す参照という例外）は `.claude/rules/modules-and-invariants.md` が持つ。**ここへ写さない。**
...
- **`pdf-core` が受け取るのは `NONE` だけである**（★★ **2026-09-14 に改めた**。#29）。**以前ここには「既定は `PROMPT`（UI）」と書いてあった**が、**画面が通るのは `assemble` と `assembleEach` であり、あちらは `EncryptionPropagation` を受け取らない**——**画面は自分で問い、答えが「続行」なら普通に呼ぶ**（§4.3.1）。`INHERIT` は実装しない（同）
