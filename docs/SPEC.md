...
%LOCALAPPDATA%\pdfjig\
  ...
  logs\                   ログ（実装済み。§10.4）
    pdfjig.0.log            現行。古いものが pdfjig.1.log → pdfjig.2.log と下がる
    pdfjig.0.log.lck        書き込み中の目印。FileHandler が作る
...
**512 KB を 3 世代。合わせて 1.5 MB を超えない。** 現行が `pdfjig.0.log` で、いっぱいになると古いものが `pdfjig.1.log` → `pdfjig.2.log` と下がり、3 つ目は捨てる。
