...
★★ **「中身が暗号化されない」ではない。** **2026-09-16 実測**——空の鍵で書いた出力は `encrypted() == true` ー AES-256 であり、**それでいて鍵なしで開けた**。PDFBox は空文字列から鍵を導いて暗号化する。**違うのは「開くのに鍵が要らない」ことである**（`EncryptionInfo#userPasswordRequired` と `Protection#userPasswordRequired`）。★ **ここを取り違えると、「保護されていません」と告げる口が、暗号化辞書もオーナーパスワードも載った出力について嘘を言う**（#30 の門の 2 段目で踏んだ）。
...
★★ **文は「pdfjig が何をしたか」で終える。** 既存の 4 値はすべて結果を述べている——「出力されたファイルは保護されていません」「取り除きました」「署名が無効になります」。**入力の性質だけを述べると、「pdfjig が止めてくれた」と読まれる。** それは**誤った安心**であり、優先順位 2 に正面から当たる。**名前も結果の形にする**——`EXTRACTION_NOT_PERMITTED` のような形は `ErrorCode.PASSWORD_REQUIRED` と同じ顔になり、**断りに見える。**
