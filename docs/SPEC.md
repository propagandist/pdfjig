...
3. **AI なしで完全に動く。** API キー未設定でも全機能が利用可能でなければならない。AI は増築であり、土台は確定的処理である。
...
**この面は、判断する側の道具には作れない。** MCP の elicitation は、仕様が「入れ子の構造と、列挙を超える配列は**意図して**扱わない」と定めている（form モード）。**しおりの木は原理的に載らない。** 仕様が用意した逃げ道は「サーバがページを配り、ブラウザで開かせる」形であり、それは**ポートを開けることを要求する**（§2.2 の Non-goal）。ホストが出すツール許可のダイアログも、**承認の対象はツール呼び出しであり、結果はその外にある。**
...
**iText は使用しない。** AGPL は Apache-2.0 での公開と両立しないためである。
...
final class Password implements AutoCloseable {
    ...
    void close();    // ゼロ埋めする。呼ぶのは作った場所だけである
}
...
★★ **置き換える前に、書けた中身をディスクへ届けさせる**（#219）。改名はメタデータであり、中身の書き込みより先に確定しうる——届く前に置き換えて控えを消すと、直後の電源断で**新旧どちらも失いうる**（NTFS の順序保証からの読み。再現はしていない）。**`pdf-core` の書き出しはすべて、閉じる前に書いたハンドルで届けさせる**（`DurableSave`。上の「既存の出力を拒む」と並ぶ `pdf-core` 全体の契約）。名前で開き直す形は、出力先の権限や他のプロセスの共有モードで**保存を新たに失敗させる**ので採らない。

★★ **届いたか確かめられなければ、警告にして、置き換えだけ止める**（判断は利用者に確かめた）。**「確かめられなかった」には 2 つが混ざり、見分けられない**——吐き出しを受け付けない場所（転送ドライブ・仮想マシンの共有フォルダなど。どこかは確かめていない）と、**本当に書けていなかった**場合（容量不足の共有フォルダ・傷んだディスク。書き込みは OS のキャッシュまで通り、書けなかったことは吐き出しで初めて分かる）である。
- **`pdf-core` は失敗させず、`Warning.NOT_DURABLE` を出す**——失敗させると、前者の場所で以前は通った書き出しが必ず失敗する。**暗号化（`Encryption`）は届いたかを伝えない**——警告を返す口が無く、既存の出力を拒むので置き換えも起きない。置き換えに使う日に口を足す
- **画面の上書き保存は、この警告が出たら置き換えない**（`ErrorCode.OUTPUT_NOT_DURABLE`）。元は手つかずで残る——後者のまま置き換えると、**欠けたものを元の名前に置き、元の控えを消して、成功と出す**
- **新しい名前への保存と分割は、書いたうえで警告として伝える**。失うものが無い。**ただし保存は「保存済み」にしない**——欠けているおそれがあるので、閉じるときに訊く

★ **分割も届けさせる**——置き換えないので得るものは無いが、`pdf-core` の契約を呼ぶ側で分けない。**遅くなる。** SSD では 1 ファイルあたり数 ms で済む。7200rpm の HDD で 1 ページずつ 200 ファイルに分けると、1.0 秒が 7.6 秒になった（2026-09-25 実測。#219 の門）。

★ **素直に置き換えない理由は、いちばんありふれた経路がそれを断るからである。** Windows の置き換えは**置き換え先が開かれていると断る。** **画面から上書き保存するとき、それを開いているのは pdfjig 自身である**（描画がそのハンドルに依存しており、保存の間は閉じられない）。#113 はそこで 2 段の置き換えに落ちており、**「開いて、直して、同じ名前で保存する」だけが弱いまま残っていた。**
...
★★ **残ったなら、その場所を画面で伝える**（#124）。失敗のダイアログに**控えの完全なパス**を出し、取り出して元の名前を付け直すよう促す。**窓に出した在り処は、入っているフォルダを「フォルダの場所をコピー」で写せる**（#137。作業場所の名前は乱数であり、書き写させると 1 桁の違いで辿り着けない。**ファイルそのものは写さない**——アドレス欄へ貼るとファイルが開く）。**伝えなければ、利用者から見えるのは「保存に失敗して、ファイルが消えた」だけである**——`.pdfjig-*` は**消してよいゴミにしか見えない**（`CLAUDE.md` 優先順位 2）。**ログには書かない**（§10.4 の★★が線を持つ）。

★★ **JVM ごと落ちた場合には届かない。** 電源断・強制終了と、**Windows のログオフ・シャットダウン**（#141）がそれに当たり、**その場では誰も何も言えない。** ★★ **次に同じフォルダへ保存したとき、保存が済んだ後に伝える**（#138）。次の保存は控えを抱えた作業場所を**見つけて残している。** **見つけたものを全部、完全なパスで窓に並べる。** 成否に関わらず出し、**利用者が消すまで毎回出る**（黙らせる手を置くと、唯一の控えが見えなくなる形に戻る）。★ **分割では伝えない**——分割は作業場所を通らない（`pdf-core` が直に書き、既存を拒む）ので、そもそも見に行かない。★ **伝えるのは pdfjig が作る形の名前（`.pdfjig-` の後ろが数字だけ）に限る。** フォルダに書ける第三者が置いたものを、pdfjig が「あなたの元のファイル」と請け合わないためである。★ **どのファイルの控えかは言えない**——作業場所は出力先の名前を持たない（持たせない理由は `OutputWorkspace` の `REPLACED`）ので、**開いて確かめるよう促す。** **ログには書かない**（§10.4。失敗ではなく、在り処は画面に出す）。★ **この道具から終わらせる経路は塞いである**——**書き出しが走っている間は、メニューの「終了」も窓の × も、その場では閉じない**（#134）。断るのではなく**覚えて待ち、書き終わってから閉じる**。**待たせる上限は置かない**——置いて切ると、**切った先がまさにこの状態である。** 待っている間は状態行に理由が出る（`CLAUDE.md` 優先順位 2）。 ★ **書き終えた後、閉じたら消える編集が残っていれば（書き出しが失敗したとき）、閉じる前に確かめる**（§7.4。#171）。 起動時に走査して拾う形は採らない——**出力先は利用者が選ぶ任意のフォルダであり、どこを走査してよいのか決まらない**（§10.1）。判断の経緯は `docs/HANDOVER.md`「決まったことの記録」にある。
...
`assemble` は `reorder` / `rotate` / `extractPages` / `deletePages` を置き換えるためのものではなく、単一の操作で済む経路はそれぞれの API を使う。

`assemble` が別に要るのは、UI が並べ替え・削除・回転を続けて行うためである。個別 API の連鎖で実装すると中間ファイルが要る。途中で失敗すれば「並べ替えた状態で回転したら並べ替えが消えた文書」が出てきて、利用者を誤解させる（§1 の設計思想、`CLAUDE.md` 優先順位 2）。UI 上の編集結果は一度の書き出しで確定させる。
...
- **暗号化** — 保護を指定しなければ出力は平文になる（§4.3）。`Warning.ENCRYPTION_NOT_PROPAGATED`。★ **保護を指定しても、ユーザーパスワードが空なら出力は誰でも開ける**（§6.1）。`Warning.CONTENT_OPENS_WITHOUT_A_KEY`
...
**★ ただし、数え上げた 6 つのうち書き換えてよいのは 3 つだけである**——しおり・文書情報・ページラベル。**それ以外は書き換えない。** 理由つきの表（正と負の両方、および出力に含めないページを指す参照という例外）は `.claude/rules/modules-and-invariants.md` が持つ。**ここへは写さず、指すだけにする。**
...
- **`pdf-core` が受け取るのは `NONE` だけである**（★★ **2026-09-14 に改めた**。#29）。**以前ここには「既定は `PROMPT`（UI）」と書いてあった。** **画面が通るのは `assemble` と `assembleEach` であり、あちらは `EncryptionPropagation` を受け取らない。** **画面は自分で問い、答えが「続行」なら普通に呼ぶ**（§4.3.1）。`INHERIT` は実装しない（同）
- **CLI は `NONE` で、警告を stderr に出力する**（★ **非対話の扱いは #185 が持つ**）
- 入力のいずれかが暗号化されていた場合、出力時に必ず警告を発する

#### 4.3.1 いつ伝えるか——書き出す前である

★★ **保護が引き継がれないことは、書き出す前に伝えて選ばせる**（#49、2026-09-12 に決めた）。**これが `PROMPT` の実装である。**

**書き出した後に伝えるのでは取り消せない。** 出力を消しても**書かれたことは元に戻らず**、**作業場所は出力先のディレクトリに作られる**ので、共有フォルダへ保存したなら**そこに平文が置かれる。** 利用者は「保護されたまま保存された」と考えて操作しており、**結果を見てから知らされる**（優先順位 2）。

★★ **訊くのは「出力に寄与する入力が暗号化されているか」である。** 開いているものが暗号化されているか、ではない——**`PdfBoxPageOperations#warnAboutContributing` が既にその粗さを却下している。** 暗号化された文書を足してから、そのページを全部消して保存すると、**出力に 1 バイトも入らないのに問われる。** 中身と食い違う警告は、**次に本物を扱ったときに無視される**（優先順位 2）。

★★ **問うのは画面である**（2026-09-14 に決めた。#29）。**答えを持っているのは画面も同じである**——**寄与する出どころは `PageSelection#sourceIndex` そのもの**であり、**鍵が要ったかどうかは `PdfDocument#openedWithPassword()` で読める**（#193 で公開した）。**追加で開き直す必要が無い。**

★★ **訂正（同日）。** ここは以前「**答えを持っているのは `pdf-core` である**」とだけ書いていた。**`pdf-core` が答えを持っているのは正しい**が、**画面も持っている**ことを見落としていた。**`pdf-core` に問わせると、入力を 2 回開くことになる。** **数えるのが包みの中、問うのが包みの外、書くのがまた包みの中**だからである。**問うのは呼ぶ側のコードを走らせることであり、包みの中では飲まれる**（#178）。**鍵の要る入力では境界の `String` も 2 倍になる**（#195）。

★★ **したがって `EncryptionPropagation` の口は増やさない。** あの列挙の Javadoc が既に定めているとおり、**`PROMPT` は画面が解決してから `pdf-core` へ渡す。** `pdf-core` は**警告を発する形のままでよい**（#49 が最初にそう書いていた側である）。

★★ **口を `WarningListener` に足す案は採らない**——**あの Javadoc は「実装は例外を投げてはならない」と定めている。** 画面が問う形なら、そもそも `pdf-core` に問い返す口が要らない。
...
★★ **伝播の指定を `assemble` と `assembleEach` へ足す必要は無くなった**（同日）。**画面が問い、答えを出してから呼ぶ**ので、**`pdf-core` へ渡るのは常に `NONE` である。** 以前ここは「指定を受け取る口がそちらにも無ければ表現できない」と書いていたが、**それは `pdf-core` が問う形を前提にしていた。**

★★ **選択肢は「中止」か「続行」だけである**（同日に決めた。#29）。**引き継ぐ道は出さない。**

★★ **保存先より先に問う**（2026-09-26、v0.0.6 の実機確認で利用者が決めた）。**何をするかを決めてから、場所を選ぶ。** 保存先を選ばせてから確認を出すと、選んだ場所を後から取り消させる形になり、何に答えているのかが読みにくかった。**分割も同じで、出力先のフォルダより先に問う。** 「パスワードで保護して保存」は、**保護の指定 → （ユーザーパスワードが空なら）この確認 → 保存先**の順である。いずれも、ファイルが書き出される前であることは変わらない。

- **引き継ぐなら、どのパスワードで暗号化し直すのかを決めなければならない。** 元のものを使い回す形は **INV-5 に真っ向から当たる**——**書き出すまで鍵を抱えることになる**
- **結合では「引き継ぐ」が 1 つに定まらない**（入力ごとにパスワードが違いうる）
- **引き継ぎたい利用者は、平文で書き出してから `protect` を掛けられる**（#28 で入った）。**少し不便だが正直である**

★★ **`EncryptionPropagation.INHERIT` は列挙に残すが、この版では実装しない。** **消さない**——**消すと「そういう考え方が無い」ことになり、次に検討するときに一から始まる。**

★ **分割は操作ごとに 1 回だけ問う**（同日に決めた）。**窓が N 回出ると「読まずに続行を押す」習慣ができる**——**下の最低条件で「次回から表示しない」を置かない理由と同じ論法**であり、**回数でそれを崩しては意味がない。**

★ **CLI は既定が `NONE` である**（上）。**非対話では問い返せないので、指定が無ければ拒む形もありうるが、既定を変えるのは別の判断である。** いまは **stderr に出す形のままにする。**

★★ **問う条件は「ユーザーパスワードが要るか」である**（`EncryptionInfo#userPasswordRequired`。§4.1）。**「暗号化されているか」ではない**——あれは暗号化辞書の有無を見るので、**オーナーパスワードだけが掛かった文書でも真になる。** 利用者は**パスワードを打たずに開いており**、§6.1 の表が「実効性＝権限フラグの保護のみ」と切り分けている側である。**書き出しを止める窓をそこで出すと、本物の機密文書に当たる前に「読まずに続行を押す」習慣ができる**——上で 1 段目の粗さを却下したのと同じ論法が、ここにも当たる。★ **読む手段は入った**（`PdfDocument#openedWithPassword()`。#193）。★★ **名前は「渡したか」であって「要るか」ではない**——**オーナーパスワードだけの文書へ鍵を渡して開いても真になる。** いま両者が一致しているのは、**画面が `PASSWORD_REQUIRED` のときにしか鍵を訊かないから**である。**`EncryptionInfo` としてどう見せるかは #180 が持つ。**

★ **選択肢の中身は決まった**（上。2026-09-14。#29）——**中止か続行かだけを訊き、引き継ぐ道は出さない。**

★★ **窓そのものには最低条件がある。** **同意が成立したかどうかが、`SECURITY.md`「対象範囲」2 番目に当たるかを決める**ようになるためであり、**どれか 1 つ崩れると、窓を出したまま経路が開く。**

- **既定のボタンと初期フォーカスは中止側である**——Enter を続けて押しても続行に倒れない
- **×・Esc・窓の外は中止である**
- ★★ **「次回から表示しない」を置かない**——置いた瞬間に、**黙って平文が落ちる経路が戻る**
- **分割は操作ごとに 1 回である**（上。2026-09-14 に決めた）

★★ **窓を出したことは免責ではない。** **同意の外で落ちる平文は、引き続き `SECURITY.md`「対象範囲」2 番目である**——下の 2 つがそれに当たる。

★★ **作業場所の置き場は動かさない。** 出力先のディレクトリに作るのは**置き換えを原子的にするため**であり（`OutputWorkspace#nextTo`）、**別ボリュームへ移すとその前提が崩れる。**

★ **ただし、時機を直しても置き場所の問題は閉じない。** **続行を選べば、平文は従来どおり同じフォルダへ落ちる。** 変わるのは**知らないまま落ちるか、選んだ結果として落ちるか**である——**前者だけを直した。**

★★ **同意の外に落ちる平文が 2 つ残る。**

- **入れ替えの途中で JVM ごと落ちると、作業場所に完成した平文が残る**（#141）。`.pdfjig-*` には**元の控えだけでなく、書けた出力そのもの**が残り、**pdfjig は在ることを告げない**——利用者は平文が書かれていないと信じる。★ **置き換えにも巻き戻しにも失敗した経路は塞いだ**（#184）。**出力が鍵の要る入力を復号した中身で、鍵なしで開けるなら**、書き出しを終えた持ち主が書けた出力を消す（`OutputWorkspace#dropWrittenOutput`）。**消し損ねたら、失敗の窓がその在り処を言う。** ★ **ふつうの出力は消さない**——開いている文書へ上書き保存して失敗したとき、編集を反映した中身はそこにしか無い。★ **次の書き出しが拾う残り物には触らない**——別の窓が退避と入れ替えの間にいると、あちらの保存を失敗させるためである
- **CLI の非対話**（#185）。**問い返せない経路をどう扱うかは決めていない。**
...
- **`String` ではなく `char[]` で保持する。** String はインターン領域に残り、ヒープダンプから回収される
- ★★ **ゼロ埋めを持つのは `Password`（§4.1）である。** 素の `char[]` を受け渡す口をこの型の中だけに置き、**作った場所で try-with-resources に載せる**（註で守る形は 3 度破れた。#135 / #144 / #145）。**受け取った側は読むだけで、消さない**——秘密が 2 本になっても、並べて宣言すれば 1 本目で投げたときに 2 本目が残らない。★★ **枠より長く生きる仕事へ渡すときは、枠ごと渡す**（`BackgroundTasks#run`）——**走り出したかどうかで持ち主が変わり、それを知っているのはその仕組みだけである**（#146）
...
**印刷禁止・コピー禁止といった権限フラグは暗号学的に強制されない。** ユーザーパスワードが空の場合、**出力は誰でも開ける**ので、権限フラグは閲覧ソフトが自主的に従っているだけの申告制である。PDFBox を含む任意のライブラリで無視できる。

★★ **「中身が暗号化されない」ではない。** **2026-09-16 実測**——空の鍵で書いた出力は `encrypted() == true` ／ AES-256 であり、**それでいて鍵なしで開けた**。PDFBox は空文字列から鍵を導いて暗号化する。**違うのは「開くのに鍵が要らない」ことである**（`EncryptionInfo#userPasswordRequired` と `Protection#userPasswordRequired`）。★ **ここを取り違えると、「保護されていません」と告げる口が、暗号化辞書もオーナーパスワードも載った出力について嘘を言う**（#30 の門の 2 段目で踏んだ）。
...
#### 6.1.1 読む側の態度——抽出を禁じるフラグが立っていたら

★★ **抽出する。そして、フラグが立っていたことを警告で告げる**（#15、2026-09-12 に決めた）。

```java
// Warning に足す値
EXTRACTED_DESPITE_RESTRICTION(
    "この文書は、作成者がテキスト抽出を禁じる設定にしています。"
        + "この設定は閲覧ソフトの自主的な遵守に依存するため、pdfjig は抽出しました。")
```

★★ **文は「pdfjig が何をしたか」で終える。** 既存の 5 値はすべて結果を述べている——「出力されたファイルは保護されていません」「誰でも開けます」「取り除きました」「署名が無効になります」。**入力の性質だけを述べると、「pdfjig が止めてくれた」と読まれる。** それは**誤った安心**であり、優先順位 2 に正面から当たる。**名前も結果の形にする**——`EXTRACTION_NOT_PERMITTED` のような形は `ErrorCode.PASSWORD_REQUIRED` と同じ顔になり、**断りに見える。**

★ **作成者を主語に置いたのは、判断材料を残すためである。** 「設定を持っています」では**誰がそう決めたのか**が落ちる。

★★ **見るのは `AccessPermissions.extractContent` である**（§4.1）。**`extractForAccessibility` ではない**——あちらは支援技術のためのもので、§6.2 が「既定で許可」と定めている。**`extractContent` が false で `extractForAccessibility` が true の文書は珍しくない。**

★★ **見るのは文書に書かれた権限であって、認証後の実効権限ではない。** PDFBox はオーナーパスワードが一致すると**すべてを許可する権限を返す**ので、実効権限を見ると**同じファイルでも、オーナーパスワードを打った回だけ警告が消える。** pdfjig のパスワード欄は 1 つで 2 種を区別しないため、**利用者にはその分岐が見えない**——見えない分岐は誤解のもとである（優先順位 2）。**作成者がそう設定したという事実は、誰が開いたかで変わらない。**

★ **警告は最善努力である。** AES-256 でも `/P` の改竄は止まらない（PDFBox は不一致を記録するだけで失敗させない）。**細工した文書は警告を消せる。**

★★ **読む手段は `PdfDocument#encryption()` である**（§4.1）。**`Encryption#inspect` ではない**——あちらは `Path` を取るので、**既に開いた文書からは呼べず、パスワードの要る文書では開き直せない。** `TextExtraction` が受け取るのは `PdfDocument` である。

★ **告げる時機はここでは決めない。** `ENCRYPTION_NOT_PROPAGATED` と同じ問題である——いまの口は書き出しが終わってから出る。★ **暗号化の側は #49 が決めた**（§4.3.1。書き出す前に問う）が、**抽出には書き出しが無い**ので、そのまま当てはまらない。**#180 が決める。**

★ **この節が決めるのは `TextExtraction` だけである。** `TableExtraction` と `Exporter`（M1）は**同じフラグの下にある**ので、そこで同じ態度を採る。**兄弟フラグ（`modify` / `assembleDocument`）は決めていない**——#28 が暗号化された文書へのページ操作を通す版で決める。

★ **値はまだ置いていない。** 誰も発しない `Warning` があると、**読む人は「どこかで発せられている」と読む。** ★★ **足すのは、`TextExtraction` が警告を発せるようになる版である**——**いまあの口には `WarningListener` が無い**（`PageOperations` には在る）。**その作業は #180 が持つ。**

**抽出しない案を採らなかった理由は、それが優先順位 2 に反するからである。** フラグに強制力は無いのに pdfjig だけが従うと、**利用者は「効いている」と思う**——上の §6.1 が名指ししている誤解を、道具の側から強めることになる。**他の閲覧ソフトでは抽出できるので、「pdfjig では抽出できない」は保護ではなく不便である。**

**黙って抽出する案も採らなかった。** 作成者が禁じていたことを知らせないと、**抽出したテキストをどう使うかの判断材料が 1 つ欠ける。**

★ **告げ方は、強制力が無いことを主語にする**——「pdfjig が制限を破った」と読まれないためである。文言は §6.1 の「閲覧ソフトの自主的な遵守に依存します」を再利用する。
...
**「PDF を追加」で足したページは、元からあったページと区別なく扱う。** 当初は「文書間のページ移動」も実装しないとしていた。それだと結合は「開いている文書とは無関係にファイルを選んで書き出す」一発勝負の操作にしかならず、他の操作と流れが揃わない。綴じる道具としての中心的な操作であり、方針を改めた。
...
**閉じたら消えるものがあるとき、消える前に一度そう言う。消えないものについては何も言わない**（#171）。

- **入口は 3 つで、どれも同じ窓を出す**——終了（メニューと窓の ×）、閉じる、別の PDF を開く。**別の PDF を開く確認は、ファイルを選んだ後に出す**（選ぶ窓を取り消しただけなら何も失わない）。起動引数から開く経路も同じ口を通る
- **消えるものは、並べ替え・回転・削除・区切り・ファイルの追加と取り外しである。** 状態行の「未保存の変更があります」とは答えている問いが違う——あちらは「書き出す内容が変わったか」で、**区切りと、ファイルの追加・取り外しは出ない。** 区切りを済みにするのは、**区切りに従って分割したとき**だけである（保存は区切りを書かない）
- **2 択である**（「保存せずに〜」／「キャンセル」）。「保存して閉じる」は出さない——保存の門を通らない口を開けることになる（#118）。**既定のボタンと初期フォーカスはキャンセル側**で、× と Esc もキャンセルになる
- **本文で「元の PDF は変更されません」と言う。** この道具は入力を書き換えないので、失うのは並べ替えの手間であってデータではない
- **保存が押せない状態（開き直しが要る状態）でも窓の形は同じ**である。キャンセルしか無い窓は「閉じられない」と同じになる
- **書き出しの最中の終了は、書き終わるのを待ってから確かめる**（§4.2）。書き出しが成功していれば消えるものが無いので訊かない。**失敗したときだけ訊く**
- ★ **Windows のログオフ・シャットダウンはこの確認を通らない**（#141）
...
**M1.5 の前半（編集の面）は M1 の表抽出より前に配る。** 理由は「表抽出の前提が変わったから」**ではない**。罫線のある表を tabula が座標ごと決定的に読むことに、変わりは無い。一方で、ページを読むモデルの出力は**照合する先が無いので、検算のしようが無い。** 優先順位 1（データを壊さない）の下では、検算できないものを主たる抽出器にはできない。**両者は競合ではなく住み分けであり、tabula の役割は縮まない。**
...
**★ 外れるのは「文書のパスとファイル名」の 1 行だけである。** その場限りのダイアログには出してよい。閉じれば消え、出す先は利用者自身が選んだ場所である。そのうえ、**伝えなければ取り戻す手立てが無い場合がある**（§4.2 の控えの在り処。`CLAUDE.md` 優先順位 2）。

**★ その在り処（入っているフォルダ）は、利用者が「フォルダの場所をコピー」を押したときだけクリップボードへ写す**（#137）。**押されない限り写さない。** 写した後に残るかどうか（Windows のクリップボード履歴、端末間の同期）は**利用者の OS の設定が決める**——**当社の設備へは届かない**ので、法務の区分（`docs/SECURITY_NOTES.md`）は動かない。
...
**1 つの例外について書くフレームは 40 まで。** 超えたぶんは数を添えて省く。上限を置かないと、`StackOverflowError` 1 件が世代を丸ごと押し出す。このエラーは 1000 を超えるフレームを持ち、**まさにここへ来る種類である。** **押し出されるのは、その直前に起きた本当の原因のほうになる**。
...
**★ API（`api.github.com`）を使わない。** 未認証の rate limit は **IP あたり 60 req/h** である。**いちばん届けたい層は、情報システム部門が MSI で一括配布した現場である。そこは NAT の内側で 1 つの外向き IP を共有する。** リダイレクト方式にこの制約は無い。JSON の解析が要らない点も同じ側にある（JDK に JSON パーサは無く、足せば配布物と CVE を見る対象が増える）。
...
**★ 確認できなかったことは記録に残す**（§10.4 の `UPDATE_NOT_CHECKED`）。画面に出るのは 1 行だけなので、**「押しても何も起きない」という報告を受けたときに読む側の手がかりがそこにしか無い。** **例外にならない失敗も残す。** 通信はできたが、返ってきたものが想定と違う場合である。**TLS を傍受するプロキシやログインページへ 302 を返すキャプティブポータルの内側がそれに当たる。そこはまさに届けたい層の環境である。**
