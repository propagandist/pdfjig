...
★★ **問うのは画面であり、`pdf-core` は警告を返す形のままである。** あちらは入力を開くまで暗号化かどうかを知らないが、**画面は既に開いている**——`DocumentSession` が持つ文書に `PdfDocument#encrypted()` を訊けば、**書き出す前に分かる。** CLI は既定が `NONE` であり、**stderr に出す形のままでよい**（問う相手が居ない）。

★ **選択肢の中身は #29 が決める。** 「引き継ぐ道を出すか、続けるかだけを訊くか」は**伝播の実装そのもの**であり、引き継ぐならパスワードをどうするかが要る。**ここが決めるのは時機だけである。**

★★ **作業場所の置き場は動かさない。** 出力先のディレクトリに作るのは**置き換えを原子的にするため**であり（§4.2 の★★）、**別ボリュームへ移すとその前提が崩れる。** **落ちる前に選べるなら、平文が共有フォルダに置かれること自体が起きない**——**時機を直せば置き場所の問題も閉じる。**
...
★ **告げる時機はここでは決めない。** `ENCRYPTION_NOT_PROPAGATED` と同じ問題であり、**#49 が持つ**——いまの口は書き出しが終わってから出る。
