...
★★ **問う条件は「ユーザーパスワードが要るか」である**（`EncryptionInfo#userPasswordRequired`。§4.1）。**「暗号化されているか」ではない**——あれは暗号化辞書の有無を見るので、**オーナーパスワードだけが掛かった文書でも真になる。** 利用者は**パスワードを打たずに開いており**、§6.1 の表が「実効性＝権限フラグの保護のみ」と切り分けている側である。**書き出しを止める窓をそこで出すと、本物の機密文書に当たる前に「読まずに続行を押す」習慣ができる**——上で 1 段目の粗さを却下したのと同じ論法が、ここにも当たる。★ **読む手段は #180 が作る。**

★ **選択肢の中身は #29 が決める。** 「引き継ぐ道を出すか、続けるかだけを訊くか」は**伝播の実装そのもの**であり、引き継ぐならパスワードをどうするかが要る。**ここが決めるのは時機だけである。**
