...
★★ **答えを持っているのは `pdf-core` である。** `assemble` は書き出す前に寄与する入力を数えており、**そこで正確に分かる。問えないのは、知らないからではなく、問い返す口が無いからである。**

★★ **口を `WarningListener` に足すことはできない。** 理由は 2 つある。**あの契約は「実装は例外を投げてはならない」と定めており**、中止を例外で返す形は契約を先に変えることになる。そして**呼び出し位置が包みの内側である**——`merge` と `writeByMerging` の `catch` が `IO_FAILURE` に包むので、**中止が「ファイルの読み書きに失敗しました」に化ける。** ★ **これは #178 が既知の限界として持っている形そのものであり、口の実装はあちらに依存する。**
...
★ **分割は 1 回の操作で N 個書く。** **何回・どの単位で問うかを決めること**——決めないと実装で分岐する。
