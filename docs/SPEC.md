...
★★ **残ったなら、その場所を画面で伝える**（#124）。失敗のダイアログに**控えの完全なパス**を出し、取り出して元の名前を付け直すよう促す。**伝えなければ、利用者から見えるのは「保存に失敗して、ファイルが消えた」だけである**——`.pdfjig-*` は**消してよいゴミにしか見えない**（`CLAUDE.md` 優先順位 2）。

★ **§10.4 の制約はここには掛からない。** あちらが禁じているのは**残り続ける記録**に文書のパスを書くことであり、**その場限りのダイアログは別である。** 出す先は利用者自身が選んだ場所であり、**伝えなければ取り戻す手立てが無い。**

★ **伝えるかどうかは、片づけを止めるのと同じ印で決める。** 別の見方を足すと、**「消さない」と「伝える」が食い違う**——消さずに残したのに何も言わない、あるいは巻き戻して元へ返したのに「残っている」と言う。

★★ **JVM ごと落ちた場合には、これは届かない。** 電源断・強制終了がそれに当たり、**控えは残るが誰も何も言わない。** 起動時に走査して拾う形は採らない——**出力先は利用者が選ぶ任意のフォルダであり、どこを走査してよいのか決まらない**（覚えているのはダイアログを始めるフォルダだけである。§10.1）。**残るのは「そもそもその状態を作らない」側の手だてだけである。**
