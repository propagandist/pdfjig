...
★★ **JVM ごと落ちた場合には届かない。** 電源断・強制終了がそれに当たり、**控えは残るが誰も何も言わない。** 判断の経緯は `docs/HANDOVER.md`「決まったことの記録」にある。
...
**★★ 掛かるのは「残り続ける記録」に対してである。** ここが守っているのは、**利用者の PC に残り、不具合の報告に貼られるファイル**の中身である。**その場限りのダイアログは対象ではない**——閉じれば消え、出す先は利用者自身が選んだ場所であり、**伝えなければ取り戻す手立てが無い場合がある**（§4.2 の控えの在り処。`CLAUDE.md` 優先順位 2）。**この但し書きをここ以外へ写さない。**
