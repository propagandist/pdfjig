...
現状: Phase 0 〜 Phase 3 まで完了。`pdf-core` は基盤型・テキスト抽出・ページ操作・描画、
`pdf-desktop` はサムネイル一覧、ドラッグ&ドロップ並べ替え、削除・回転・範囲指定、
結合・分割、パスワード入力までが動く。次は Phase 4（リリース）。
着手前に下の未決事項のうちアプリ名・アイコン・最小 Windows バージョンの確認が要る。
...
- [ ] `SPEC.md` §4.2 にない `PageOperations.assemble(Path, List<PageSelection>, Path)` と
      ...
      `deletePages` はそのまま残してある。仕様側を直すべきなら `SPEC.md` に追記したい
- [ ] 上に伴い、UI では回転も編集セッションに含めた。`HANDOVER.md` 3-3 は回転を
...
- [ ] Phase 4 の jpackage に必要な情報が揃っていない。アプリケーション名の表示、
      アイコン、ベンダー名、アップグレード用の UUID を決める必要がある
