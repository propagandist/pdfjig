...
現状: リポジトリ (https://github.com/propagandist/pdfjig) は作成済み、中身は空。
...
- [ ] パッケージ名を `com.propagandist.pdfjig` とするか、`io.github.propagandist.pdfjig` に統一するか
      （Maven Central 公開時の groupId は `io.github.propagandist` で確定しているため、
      最初から揃えておくほうが後の改名を避けられる）
