...
.DESCRIPTION
    使い捨ての Windows を検証の土台にする。手元で uiTest を走らせてもホストの build/ と
    ~/.gradle を汚さず、画面をロックしたままでも走る。インストーラを入れて消しても
    ホストに残らない。

    ★ uiTest ではカーソルが奪われる。最小化すればカーソルは守られるが、今度は入力が
    届かなくなって大半が落ちる（2026-08-23 実測。docs/HANDOVER.md「uiTest を手元で
    隔離しようとして、できないと分かった」に 3 構成の値がある）。
    マウスを使わない検証（InstallCheck）はこの制約と無関係に働く。
