...
.DESCRIPTION
    ...
    開発機を 2 分ほど取り上げられ、画面をロックしていると落ちる。Sandbox を通すと、ホストの
    build/ と ~/.gradle を汚さず、画面をロックしたままでも走る。

    ★ カーソルは奪われる。窓は前面のままにしておくこと——最小化すると入力が届かなくなり、
    大半が落ちる（2026-08-23 実測。docs/HANDOVER.md「uiTest を手元で隔離しようとして、
    できないと分かった」に 3 構成の値がある）。1 つの Windows セッションの中では避けられない。
