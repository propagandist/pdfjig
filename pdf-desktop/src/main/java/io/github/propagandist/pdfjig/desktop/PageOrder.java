...
public final class PageOrder {
    ...
    /**
     * 書き出した並びから変わっているか。まだ書き出していなければ、開いた直後の並びと比べる。
     *
     ...
     * 書き出した並びを、以後の基準にする。
     *
     * <p><b>いまの並びではなく、書き出した並びを受け取る。</b> 書き出しは非同期であり、
     * その間に利用者が並べ替えていれば、その分はまだ書き出されていない。
     * いまの並びを基準にすると、保存していない編集まで保存済みに見える。
     *
     * @param saved 書き出した並び
     */
    public void markSaved(List<PageSelection> saved) {
        baseline.clear();
        baseline.addAll(saved);
    }

    /**
     * 最後に書き出した状態に戻す。まだ書き出していなければ、開いた直後の状態に戻す。
     *
