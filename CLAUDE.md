...
| 表記の対 | **既定は短形**（長音を省く）。`サーバ` `フォルダ` `メモリ` がそうで、**画面に出る文言も `フォルダ` である**（**2026-09-09 実測**）。★ **`ユーザー` だけは長形のまま据える**（18 件）。★★ **決めたのは新しい語の寄せ先であって、既存の書き換えではない。** 例外を足すときはこの行へ書く |
| 定番語彙と、避ける語の対象外リスト | **`正本` `実測` `実走` `門` `効く` `落ちる` `腐る` `縛る` `掴む`**（**2026-09-09 実測**。`実測` 86 件・`門` 46 件・`落ちる` 37 件が多い）。★ **このうち `効く` `落ちる` `実走` `腐る` `掴む` `素通り` は、よその口語動詞の表では言い換え対象になる。ここでは規約語であり、対象外とする。** 上の「命名」はコードの識別子の話で、**文章の語彙とは別** |
...
| 基準時点の書式 | **2 形を使い分ける。** `YYYY-MM-DD 実測` は**自分で回して得た出力**（コマンド・CI の run・画面）、`YYYY-MM-DD 時点` は**自分では回していない外の記載や状態**（ベンダのページ・他所の文書）。★ **実績もそうなっている**——実測 32 件に対し時点は 1 件で、それは #43 が引いたベンダのページである（**2026-09-09 実測**） |
