...
| 表記の対 | **既定は長形**（長音を付ける）。**実績も長形が多い**——`ランナー` 26 件・`ユーザー` 18 件・`ツールバー` 18 件ほかで、**長形 71 / 短形 48**（**2026-09-09 実測**。下記 6 ファイル）。★ **`サーバ` `フォルダ` `メモリ` の 3 語だけ短形で据える**（8 / 31 / 5 件。**画面に出る文言も `フォルダ` である**）。★★ **決めたのは新しい語の寄せ先であって、既存の書き換えではない**——**揺れている 3 語**（`ユーザ` 1 件・`ベンダ` 2 件・`ランチャ` 1 件）**の寄せ先は長形だが、直すのは推敲の作業である。** 例外を足すときはこの行へ書く |
| 定番語彙と、避ける語の対象外リスト | **`正本` `実測` `実走` `門` `効く` `落ちる` `腐る` `縛る` `掴む` `素通り`**（**2026-09-09 実測**。`実測` 95 件・`落ちる` 40 件・`門` 36 件が多い。★ **`門` は単独の語で数える**——`部門` 5 件と `関門` 7 件は別語である）。★ **このうち `効く` `落ちる` `実走` `腐る` `掴む` `素通り` は、よその口語動詞の表では言い換え対象になる。ここでは規約語であり、対象外とする。** 上の「命名」はコードの識別子の話で、**文章の語彙とは別** |
| 一文の長さの閾値 | **60 字。超えたら候補として見る**（落とすのではない。org §1）。**根拠は分布の実測**——`docs/*.md` ＋ `README.md` ＋ `CLAUDE.md` ＋ `SECURITY.md` の 6 ファイルで **n=3804 ／ 中央値 25 ／ p90 45 ／ 最大 106 ／ 60 字超 85 件 2.2%**（**2026-09-09 実測**）。★ **p90 の 1.3 倍に置いてある**——**いま普通に書けている文が赤くならない位置である。** ★ **対象を変えると数字が変わるので、対象範囲ごと書く**（85 件のうち 48 件が `docs/SPEC.md` にある） |
| 基準時点の書式 | **2 形を使い分ける。** `YYYY-MM-DD 実測` は**自分で回して得た出力**（コマンド・CI の run・画面）、`YYYY-MM-DD 時点` は**自分では回していない外の記載や状態**（ベンダーのページ・他所の文書）。★ **実績もそうなっている**——実測 32 件に対し時点は 1 件で、それは #43 が引いたベンダのページである（**2026-09-09 実測**） |
| 校正の実装と強度 | **無い。** `build.yml` は `paths-ignore` に `**/*.md` を持つので、文書だけの変更では回らない。検査は手元 |
| 固有の観点 | **2 つ。** ★★ **① 確かめていないことを、確かめたように書かない。**「機械が見る」と書くなら、**その筋をわざと落として確かめてから書く**——`v0.1.2` だけで 5 回踏んだ（#144 が 2 回、#145 ／ #44 ／ #43 が各 1 回）。★ **② 配布物の文言は、前の版を開いてから書く**（`docs/RELEASE_NOTES.md` ／ CLI の出力 ／ 画面）。**v0.1.0 の捨てタグで、機能表に「1 ページずつに分割」が無いことが見つかっている**（#27） |
