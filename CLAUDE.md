...
| 表記の対 | **対象は、長音を付けるか省くかで揺れる語だけ**（`-er` / `-ar` / `-or` に当たるもの）。**既定は長形。** ★ **短形で据えるのは `サーバ` `フォルダ` `メモリ` `インストーラ` `プロバイダ` `ヘッダ` の 6 語**（**画面に出る文言も短形である**）。★ **`リポジトリ` `ライブラリ` のような `-y` 語尾は対象外**——**この文書群に長形が 1 件も無く、揺れていない。** ★★ **決めたのは寄せ先であって、既存の書き換えではない。** 実測・揺れている語・例外の根拠は `docs/HANDOVER.md`（#81）。**例外を足すときはこの行へ書く** |
| 定番語彙と、避ける語の対象外リスト | **`正本` `実測` `実走` `門` `効く` `落ちる` `腐る` `縛る` `掴む` `素通り` の 10 語。足すときはこの行へ書く。** ★ **避ける語のリストは持たない**（org §0。よその表を持ち込まない）。**ここが持つのは、口語に見えて規約語である 6 語だけである**——`効く` `落ちる` `実走` `腐る` `掴む` `素通り`。上の「命名」はコードの識別子の話で、**文章の語彙とは別** |
| 一文の長さの閾値 | **80 字。超えたら候補として見る**（落とすのではない。org §1）。★★ **掛かるのは 1 文であって、折り返した 1 行ではない**——**org §8 の 5 本目は行ごとに切るので、折り返しで割れた文は出ない。測るときは折り返しを繋いでから `。` で切る**（あの検査は表の行も見ない）。**根拠は分布の実測**——`docs/*.md` ＋ `README.md` ＋ `CLAUDE.md` ＋ `SECURITY.md` の 6 ファイルを文として測ると **n=2502 ／ 中央値 34 ／ p90 74 ／ 最大 209 ／ 80 字超 172 件 6.9%**（**2026-09-09 実測**。木は `05fc758`）。★ **p90 の上に置いてある。対象範囲と木ごと書くこと** |
