...
2. **敵対的に読む** — 観点は固定する: 上の「セキュリティ」節が指す先 ／ INV-1〜6 ／
   ...
   **PR ごとに必ず `/code-review high`。★ レベルを省かない**——省くと前回の値が再利用される。
   ★ **`/security-review` はマイルストーン単位**（2026-09-26 に改めた）——捨てタグの前に、前回かけた位置から
   `develop` の先端までを**モジュールごとに分けて**かける。範囲と結果は「vX.Y.Z をリリースする」issue に書く。
...
見本は #65。「読んだ」には `/security-review` はマイルストーン単位、と書く）。★★ **`門:` の範囲より後のコミットは見られていない。直前に head へ更新する。**
